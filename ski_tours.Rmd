...
p_load(gsheet, knitr,raster,leaflet,rsconnect,datasets, tidyverse,  maptools, geojsonio,
  rgdal, ggplot2, devtools, ggthemes, gtools, scales, DT, RColorBrewer, Rserve, leaflet.extras, sf)

all<-geojson_read("ski_tours/all_ski_tours.geojson", what="sp") %>% st_as_sf()

#load new tour geojson tracks
#edith<-geojson_read("ski_tours/edith creek rainier.geojson", what="sp") %>% st_as_sf() %>%
 # mutate(name= "edith creek", id=45)
#rlp<-geojson_read("ski_tours/red lake peak.geojson", what="sp") %>% st_as_sf() %>%
#  mutate(name= "red lake peak", id=46)
#new <- rbind(tallac, talking, echo, rlp, edith) %>%
  #select(id, name)

#combine the new tracks with the all the other
#all <- rbind(all, new)

#st_write(shapefile, dsn="ski_tours/all_ski_tours.geojson", driver="GeoJSON")
...
leaflet(width=900, height=700) %>%
 # addMarkers(
  ...
  addPopups(-120.040271, 38.861944, "recent trip", options = popupOptions(closeButton = FALSE)) %>%
...
data_table<-all1 %>% data.frame() %>% dplyr::select(date, general.area, tour.name, nwac.avy.rating, weather, duration, elevation.gain..ft., distance..mile., photos )
