...
library(pacman)
pacman::p_load(raster,dplyr,gsheet, leaflet, shiny,knitr, rsconnect, rgdal, datasets, ggplot2, ggthemes, gtools, scales, DT, lubridate)
...
bird$Date1<-mdy(bird$Date)
#bird$Date1<-format(bird$Date1, "%B %d, %Y")

lat_recent<-bird%>% filter (Date1==max(bird$Date1)) %>% select (lat)
lat_recent<-lat_recent[1,]
lat_recent <-lat_recent + .005

lon_recent<-bird%>% filter (Date1==max(bird$Date1)) %>% select (lon)
...
leaflet (width=900, height=700) %>%
 addPopups(lng=lon_recent, lat = lat_recent, "recent observation", options = popupOptions(closeButton = FALSE)) %>%
   ...
   addEasyButton(easyButton(icon="fa-globe", title="Zoom to Level 1", onClick=JS("function(btn, map){ map.setZoom(3); }"))) %>%
    addLegend("topright", title= "Recent Bird Observations_test", colors= "#3487db", labels= "birds observed")

```
```{r message=FALSE, warning=FALSE}

bird1$Date<-mdy(bird1$Date)
