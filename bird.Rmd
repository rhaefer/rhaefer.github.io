...
ui <- dashboardPage(
  dashboardHeader(),
  dashboardSidebar(width=300,selectInput("slider","Select Observation",choices=unique(bird$id))),
  dashboardBody(fluidRow(box(leafletOutput("map")),
                         box(htmlOutput("bird_info")))))
server <- function(input, output) { 
   output$map<- renderLeaflet({
   bird %>% st_as_sf(coords=c("Longitude","Latitude")) %>% 
     filter(id==input$slider)  %>%
     leaflet() %>% addCircles() %>% addTiles()
   })
    output$bird_info<-renderText({
     req(input$slider)
    dob <- paste(strong("Date: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(Date))
    name <- paste(strong("Common Name: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(Common.Name))
     name1 <- paste(strong("Scientific Name: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(Scientific.Name))
     count <- paste(strong("Number Observed: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(Count))
     state <- paste(strong("State: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(State.Province))
     loc <- paste(strong("Location: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(Location))
     ebird <- paste(strong("More Info: "), bird %>% 
                   filter(id==input$slider) %>% 
                   select(html))
    paste(dob, name,name1,count,state,loc,ebird,sep = "<br>")
  })
     }
shinyApp(ui, server)
```
