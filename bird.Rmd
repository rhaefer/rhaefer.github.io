...
bird$Date1<-format(bird$Date, "%B %d, %Y")

lat_recent<-bird%>% filter (Date==max(bird$Date)) %>% select (lat)
lat_recent<-lat_recent[1,]
lat_recent <-lat_recent + .002

lon_recent<-bird%>% filter (Date==max(bird$Date)) %>% select (lon)
...
leaflet (width=900, height=700) %>%
 addPopups(lon_recent, lat = lat_recent, "recent observation", options = popupOptions(closeButton = FALSE)) %>%
