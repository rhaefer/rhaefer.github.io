...
bird_table<-data.frame(bird %>% group_by(Common.Name, html) %>% summarise(Count=sum(Count))) 
bird_table<-bird_table %>% 
      rename(`Common Name`=Common.Name, `Number Observed`=Count, `More Information`=html) %>% 
      select(`Common Name`, `Number Observed`, `More Information`)
kable(bird_table, caption = "Summary of Recent Bird Observations", row.names=FALSE, align='l')
